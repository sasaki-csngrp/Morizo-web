...
export async function POST(request: NextRequest) {
  const timer = ServerLogger.startTimer('subscription-update-api');
  
  try {
    ServerLogger.info(LogCategory.API, 'プラン更新API呼び出し開始');
    ...
    ServerLogger.info(LogCategory.API, 'Morizo AIにプラン更新リクエスト送信開始');
    
    const url = `${MORIZO_AI_URL}/api/subscription/update`;
    ...
    timer();
    logApiCall('POST', '/api/subscription/update', 200, undefined);
    ...
    timer();
    logError(LogCategory.API, error, 'subscription-update-api');
    logApiCall('POST', '/api/subscription/update', 500, undefined, error instanceof Error ? error.message : '不明なエラー');
