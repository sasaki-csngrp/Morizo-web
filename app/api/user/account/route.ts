...
export async function DELETE(request: NextRequest) {
  const timer = ServerLogger.startTimer('user-account-delete-api');
  
  try {
    ServerLogger.info(LogCategory.API, 'アカウント削除API呼び出し開始');
    ...
    ServerLogger.info(LogCategory.API, 'Morizo AIにアカウント削除リクエスト送信開始');
    
    const url = `${MORIZO_AI_URL}/api/user/account`;
    ...
    timer();
    logApiCall('DELETE', '/api/user/account', 200, undefined);
    
    const nextResponse = NextResponse.json({
      success: data.success,
      message: data.message || 'アカウントが正常に削除されました'
    });
    ...
    timer();
    logError(LogCategory.API, error, 'user-account-delete-api');
    logApiCall('DELETE', '/api/user/account', 500, undefined, error instanceof Error ? error.message : '不明なエラー');
