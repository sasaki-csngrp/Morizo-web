...
export async function PUT(
  ...
  const timer = ServerLogger.startTimer('recipe-rating-update-api');
  
  try {
    const { history_id: historyId } = await params;
    ServerLogger.info(LogCategory.API, 'レシピ評価更新API呼び出し開始', { historyId });
    ...
    ServerLogger.info(LogCategory.API, 'Morizo AIにレシピ評価更新リクエスト送信開始');
    
    const url = `${MORIZO_AI_URL}/api/menu/history/${historyId}/rating`;
    ...
    timer();
    logApiCall('PUT', `/api/menu/history/${historyId}/rating`, 200, undefined);
    ...
    timer();
    const { history_id: historyId } = await params;
    logError(LogCategory.API, error, 'recipe-rating-update-api');
    logApiCall('PUT', `/api/menu/history/${historyId}/rating`, 500, undefined, error instanceof Error ? error.message : '不明なエラー');
